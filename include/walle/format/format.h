...
#include <walle/buffer/stack_buffer.h>
...
#if (WALLE_HAS_INCLUDE(<string_view>) && __cplusplus > 201402L)
# include <string_view>
# define WALLE_HAS_STRING_VIEW 1
#else
# define WALLE_HAS_STRING_VIEW 0
#endif
...
// only define WALLE_BUILTIN_CLZ using the MSVC intrinsics
...
typedef std::numeric_limits<walle::internal::DummyInt> FPUtil;
...
template <>
class numeric_limits<walle::internal::DummyInt> :
 ...
 public:
  ...
  static bool isinfinity(T x) {
    ...
    // isinf macro > std::isinf > ::isinf > walle::internal::isinf
...
WALLE_EXTENSION typedef long long LongLong;
WALLE_EXTENSION typedef unsigned long long ULongLong;
...
template <typename Impl, typename Char, typename Spec = walle::FormatSpec>
class BasicPrintfArgFormatter;

template <typename CharType,
          typename ArgFormatter = walle::ArgFormatter<CharType> >
...
class FormatError : public std::runtime_error {
 public:
  ...
  ~FormatError() WALLE_NOEXCEPT WALLE_OVERRIDE;
};
...
inline typename MakeUnsigned<Int>::Type to_unsigned(Int value) {
  WALLE_ASSERT(value >= 0, "negative value");
...
// A fixed-size buffer.
template <typename Char>
class FixedBuffer : public walle::BasicBuffer<Char> {
 public:
  FixedBuffer(Char *array, std::size_t size) : walle::BasicBuffer<Char>(array, size) {}

 protected:
  void grow(std::size_t size) WALLE_OVERRIDE;
};
...
WALLE_NORETURN void report_unknown_type(char code, const char *type);
...
#ifdef WALLE_BUILTIN_CLZLL
// Returns the number of decimal digits in n. Leading zeros are not counted
// except for n == 0 in which case count_digits returns 1.
inline unsigned count_digits(uint64_t n) {
  // Based on http://graphics.stanford.edu/~seander/bithacks.html#IntegerLog10
  // and the benchmark https://github.com/localvoid/cxx-benchmark-count-digits.
  int t = (64 - WALLE_BUILTIN_CLZLL(n | 1)) * 1233 >> 12;
...
#ifdef WALLE_BUILTIN_CLZ
// Optional version of count_digits for better performance on 32-bit platforms.
inline unsigned count_digits(uint32_t n) {
  int t = (32 - WALLE_BUILTIN_CLZ(n | 1)) * 1233 >> 12;
...
class ThousandsSep {
 private:
  walle::StringRef sep_;

  // Index of a decimal digit with the least significant digit having index 0.
  unsigned digit_index_;

 public:
  explicit ThousandsSep(walle::StringRef sep) : sep_(sep), digit_index_(0) {}
...
// allow storage in internal::StackBuffer.
struct Arg : Value {
  Type type;
};
...
struct ConvertToInt {
  enum {
    enable_conversion = sizeof(walle::internal::convert(get<T>())) == sizeof(Yes)
  };
...
inline walle::StringRef thousands_sep(...) { return ""; }
...
void format_arg(Formatter&, ...) {
  WALLE_STATIC_ASSERT(FalseType<Formatter>::value,
                    "Cannot format argument. To enable the use of ostream "
                    "operator<< include ostream.h. Otherwise provide "
...
class MakeValue : public Arg {
 ...
 private:
  ...
  //   walle::format("{}", L"test");
  // To fix this, use a wide format string: walle::format(L"{}", L"test").
...
class RuntimeError : public std::runtime_error {
 protected:
  ...
  ~RuntimeError() WALLE_NOEXCEPT WALLE_OVERRIDE;
};
...
/**
  ...
  To use `~walle::ArgVisitor` define a subclass that implements some or all of the
  visit methods with the same signatures as the methods in `~walle::ArgVisitor`,
  for example, `~walle::ArgVisitor::visit_int()`.
  Pass the subclass as the *Impl* template parameter. Then calling
  `~walle::ArgVisitor::visit` for some argument will dispatch to a visit method
  specific to the argument type. For example, if the argument type is
  ``double`` then the `~walle::ArgVisitor::visit_double()` method of a subclass
  will be called. If the subclass doesn't contain a method with this signature,
  then a corresponding method of `~walle::ArgVisitor` will be called.

  **Example**::

    class MyArgVisitor : public walle::ArgVisitor<MyArgVisitor, void> {
     public:
      void visit_int(int value) { walle::print("{}", value); }
      void visit_double(double value) { walle::print("{}", value ); }
    };
  \endrst
 */
...
class ArgVisitor {
 ...
 public:
  ...
  /**
    ...
    the `~walle::ArgVisitor::visit_double()` method of the *Impl* class will be
    called.
    \endrst
   */
  Result visit(const Arg &arg) {
    ...
    case Arg::NONE:
    case Arg::NAMED_ARG:
      WALLE_ASSERT(false, "invalid argument type");
      break;
    case Arg::INT:
...
class ArgMap {
 private:
  typedef std::vector<
    std::pair<walle::BasicStringRef<Char>, internal::Arg> > MapType;
 ...
 public:
  ...
  const internal::Arg *find(const walle::BasicStringRef<Char> &name) const {
...
void ArgMap<Char>::init(const ArgList &args) {
  ...
  const NamedArg *named_arg = WALLE_NULL;
  bool use_values =
...
class ArgFormatterBase : public ArgVisitor<Impl, void> {
 private:
  ...
  WALLE_NON_COPYABLE(ArgFormatterBase);
 ...
 public:
  ...
  void visit_char(int value) {
    ...
    if (spec_.align_ == ALIGN_NUMERIC || spec_.flags_ != 0)
      WALLE_THROW(FormatError("invalid format specifier for char"));
...
/**
  ...
  To use `~walle::BasicArgFormatter` define a subclass that implements some or
  all of the visit methods with the same signatures as the methods in
  `~walle::ArgVisitor`, for example, `~walle::ArgVisitor::visit_int()`.
  Pass the subclass as the *Impl* template parameter. When a formatting
  function processes an argument, it will dispatch to a visit method
  specific to the argument type. For example, if the argument type is
  ``double`` then the `~walle::ArgVisitor::visit_double()` method of a subclass
  will be called. If the subclass doesn't contain a method with this signature,
  then a corresponding method of `~walle::BasicArgFormatter` or its superclass
  will be called.
  \endrst
 */
template <typename Impl, typename Char, typename Spec = walle::FormatSpec>
...
class BasicFormatter : private internal::FormatterBase {
 ...
 private:
  ...
  WALLE_NON_COPYABLE(BasicFormatter);
...
#if WALLE_USE_VARIADIC_TEMPLATES
...
# define FMT_ASSIGN_char(n) \
  arr[n] = walle::internal::MakeValue< walle::BasicFormatter<char> >(v##n)
# define FMT_ASSIGN_wchar_t(n) \
  arr[n] = walle::internal::MakeValue< walle::BasicFormatter<wchar_t> >(v##n)
...
# define FMT_VARIADIC_VOID(func, arg_type) \
  ...
  void func(arg_type arg0, const Args & ... args) { \
    typedef walle::internal::ArgArray<sizeof...(Args)> ArgArray; \
    typename ArgArray::Type array{ \
      ArgArray::template make<walle::BasicFormatter<Char> >(args)...}; \
    func(arg0, walle::ArgList(walle::internal::make_type(args...), array)); \
  }
...
# define FMT_VARIADIC_CTOR(ctor, func, arg0_type, arg1_type) \
  ...
  ctor(arg0_type arg0, arg1_type arg1, const Args & ... args) { \
    ...
    typename ArgArray::Type array{ \
      ArgArray::template make<walle::BasicFormatter<Char> >(args)...}; \
    func(arg0, arg1, walle::ArgList(walle::internal::make_type(args...), array)); \
  }

#else

# define FMT_MAKE_REF(n) \
  walle::internal::MakeValue< walle::BasicFormatter<Char> >(v##n)
...
# define FMT_WRAP1(func, arg_type, n) \
  ...
  inline void func(arg_type arg1, FMT_GEN(n, FMT_MAKE_ARG)) { \
    const walle::internal::ArgArray<n>::Type array = {FMT_GEN(n, FMT_MAKE_REF)}; \
    func(arg1, walle::ArgList( \
      walle::internal::make_type(FMT_GEN(n, FMT_MAKE_REF2)), array)); \
  }

// Emulates a variadic function returning void on a pre-C++11 compiler.
# define FMT_VARIADIC_VOID(func, arg_type) \
  inline void func(arg_type arg) { func(arg, walle::ArgList()); } \
...
class SystemError : public internal::RuntimeError {
 ...
 public:
  /**
   \rst
   Constructs a :class:`walle::SystemError` object with a description
   formatted with `walle::format_system_error`. *message* and additional
   ...
   **Example**::
     ...
     if (!file)
       throw walle::SystemError(errno, "cannot open file '{}'", filename);
   \endrst
  */
  ...
  ~SystemError() WALLE_NOEXCEPT WALLE_OVERRIDE;
...
/**
  ...
  *error_code* is a system error code as given by ``errno``.
  ...
  \endrst
 */
void format_system_error(walle::Writer &out, int error_code,
                                 walle::StringRef message) WALLE_NOEXCEPT;

/**
  \rst
  This template provides operations for formatting and writing data into
  a character stream. The output is stored in a buffer provided by a subclass
  such as :class:`walle::BasicMemoryWriter`.
...
class BasicWriter {
 private:
  // Output buffer.
  walle::BasicBuffer<Char> &buffer_;

  WALLE_NON_COPYABLE(BasicWriter);
 ...
 protected:
   ...
   */
  explicit BasicWriter(walle::BasicBuffer<Char> &b) : buffer_(b) {}
 ...
 public:
   ...
   */
  const Char *data() const WALLE_NOEXCEPT { return buffer_.data(); }
  ...
  /**
    ...
    \endrst
   */
  BasicWriter &operator<<(walle::BasicStringRef<Char> value) {
  ...
  void clear() WALLE_NOEXCEPT { buffer_.clear(); }

  walle::BasicBuffer<Char> &buffer() WALLE_NOEXCEPT { return buffer_; }
};
...
void BasicWriter<Char>::write_str(
  ...
  if (str_size == 0) {
    if (!str_value) {
      WALLE_THROW(FormatError("string pointer is null"));
    }
  }
...
class BasicMemoryWriter : public BasicWriter<Char> {
 private:
  walle::StackBuffer<Char, internal::INLINE_BUFFER_SIZE, Allocator> buffer_;
 ...
 public:
  ...
  /**
    \rst
    Constructs a :class:`walle::BasicMemoryWriter` object moving the content
...
/**
  ...
  use :class:`walle::BasicMemoryWriter`.
...
class BasicArrayWriter : public BasicWriter<Char> {
 ...
 public:
  /**
   \rst
   Constructs a :class:`walle::BasicArrayWriter` object for *array* of the
   given size.
   \endrst
   */
...
/**
  ...
  Example:
    print_colored(walle::RED, "Elapsed time: {0:.2f} seconds", 1.23);
 */
...
#define FMT_FOR_EACH_(N, f, ...) \
  FMT_EXPAND(WALLE_PREPROCESSOR_JOIN(FMT_FOR_EACH, N)(f, __VA_ARGS__))
...
# define FMT_VARIADIC_(Const, Char, ReturnType, func, call, ...) \
  ...
  ReturnType func(FMT_FOR_EACH(FMT_ADD_ARG_NAME, __VA_ARGS__), \
    ...
    call(FMT_FOR_EACH(FMT_GET_ARG_NAME, __VA_ARGS__), \
      walle::ArgList(walle::internal::make_type(args...), array)); \
  }
#else
...
# define FMT_WRAP(Const, Char, ReturnType, func, call, n, ...) \
  ...
  inline ReturnType func(FMT_FOR_EACH(FMT_ADD_ARG_NAME, __VA_ARGS__), \
    ...
    walle::internal::ArgArray<n>::Type arr; \
    FMT_GEN(n, FMT_ASSIGN_##Char); \
    call(FMT_FOR_EACH(FMT_GET_ARG_NAME, __VA_ARGS__), walle::ArgList( \
      walle::internal::make_type(FMT_GEN(n, FMT_MAKE_REF2)), arr)); \
  }
...
#endif  // WALLE_USE_VARIADIC_TEMPLATES
...
/**
  ...
  **Example**::
    ...
    void print_error(const char *file, int line, const char *format,
                     walle::ArgList args) {
      walle::print("{}: {}: ", file, line);
      walle::print(format, args);
    }
...
#define FMT_CAPTURE_ARG_(id, index) ::walle::arg(#id, id)

#define FMT_CAPTURE_ARG_W_(id, index) ::walle::arg(L###id, id)

/**
  \rst
  Convenient macro to capture the arguments' names and values into several
  ``walle::arg(name, value)``.
...
unsigned parse_nonnegative_int(const Char *&s) {
  ...
  // Convert to unsigned to prevent a warning.
  if (value > max_int)
    WALLE_THROW(FormatError("number is too big"));
  return value;
}
...
inline void require_numeric_argument(const Arg &arg, char spec) {
  if (arg.type > Arg::LAST_NUMERIC_TYPE) {
    std::string message =
        walle::format("format specifier '{}' requires numeric argument", spec);
    WALLE_THROW(walle::FormatError(message));
  }
}
...
void check_sign(const Char *&s, const Arg &arg) {
  ...
  if (arg.type == Arg::UINT || arg.type == Arg::ULONG_LONG) {
    WALLE_THROW(FormatError(walle::format(
...
const Char *BasicFormatter<Char, ArgFormatter>::format(
  ...
  if (*s == ':') {
      ...
      do {
        ...
        if (spec.align_ != ALIGN_DEFAULT) {
          if (p != s) {
            if (c == '}') break;
            if (c == '{')
              WALLE_THROW(FormatError("invalid fill character '{'"));
            s += 2;
            spec.fill_ = c;
          } else ++s;
    ...
    if ('0' <= *s && *s <= '9') {
      ...
      if (*s++ != '}')
        WALLE_THROW(FormatError("invalid format string"));
      ULongLong value = 0;
      switch (width_arg.type) {
      case Arg::INT:
        if (width_arg.int_value < 0)
          WALLE_THROW(FormatError("negative width"));
      ...
      case Arg::ULONG_LONG:
        ...
        break;
      default:
        WALLE_THROW(FormatError("width is not integer"));
      }
      unsigned max_int = (std::numeric_limits<int>::max)();
      if (value > max_int)
        WALLE_THROW(FormatError("number is too big"));
    ...
    if (*s == '.') {
      ...
      if ('0' <= *s && *s <= '9') {
        ...
        switch (precision_arg.type) {
          case Arg::INT:
            if (precision_arg.int_value < 0)
              WALLE_THROW(FormatError("negative precision"));
          ...
          case Arg::ULONG_LONG:
            ...
            break;
          default:
            WALLE_THROW(FormatError("precision is not integer"));
        }
        unsigned max_int = (std::numeric_limits<int>::max)();
        if (value > max_int)
          WALLE_THROW(FormatError("number is too big"));
        ...
        WALLE_THROW(FormatError("missing precision specifier"));
      }
      if (arg.type <= Arg::LAST_INTEGER_TYPE || arg.type == Arg::POINTER) {
        WALLE_THROW(FormatError(
            walle::format("precision not allowed in {} format specifier",
...
void BasicFormatter<Char, AF>::format(BasicCStringRef<Char> format_str) {
  ...
  while (*s) {
    ...
    if (c == '}')
      WALLE_THROW(FormatError("unmatched '}' in format string"));
...
void format_arg(walle::BasicFormatter<Char, ArgFormatter> &f,
  ...
  if (it != e.last) {
    ...
    f.format(format_str, internal::MakeArg<walle::BasicFormatter<Char, ArgFormatter> >(*it++));
    while (it != e.last) {
...
#if WALLE_USE_USER_DEFINED_LITERALS
namespace walle {
namespace internal {
...
/**
  \rst
  C++11 literal equivalent of :func:`walle::format`.

  **Example**::

    using namespace walle::literals;
...
/**
  \rst
  C++11 literal equivalent of :func:`walle::arg`.
...
#endif // WALLE_USE_USER_DEFINED_LITERALS
...
# include <walle/format/format_inl.h>


#endif  // WALLE_FORMAT_FORMAT_H_
